...
class Formity {
  ...
  public static function getInstance($cdr = null) {
    ...
    #Theme::JS('/libs/js/formity.js', 'formity');
    return $rp;
  }
...
//  }
  ...
  function buildButtons() {
    ...
    foreach($this->buttons as $b) {
      $html .= '<button type="submit" name="formity_button" class="btn btn-primary mb-2">';
      $html .= $b;
      $html .= '</button>';
    }
    return $html;
  ...
  private static function _renderFormity($form, $nivel = 0) {
    ...
    if(!empty($form->error)) {
      $rp .= '<article class="card border-danger"><div class="card-header bg-danger text-white">Debes seguir estas indicaciones:</div>';
      $rp .= '<div class="card-body"><ul style="margin-top:0px;">';
...
#        $rp .= '<div class="column is-' . $field->size . '">';
        $rp .= '<div class="form-group">';
        if($field->isForm()) {
          $rp .= '<label class="label">' . $field->name . '</label><div>';
          if($field->childrangchange) {
            ...
            $rp .= '<a class="button is-small is-rounded" id="addMore_' . $field->getNameRequest() . '"><span class="icon"><i class="fa fa-plus fa-1x"></i></span></a>';
          }
          $rp .= '</div>';
          $rp .= $field->render();
        }
...
#    $rp .= '</div>';
  ...
  function render() {
    $rp = '';
    if(false && !empty(Theme::data('submenu'))) {
  ...
  function renderInPage() {
    $rp = "<div class=\"card shadow mb-4\">";
    if(!is_null($this->title)) {
      $rp .= "<div class=\"card-header py-3\"><h6 class=\"m-0 font-weight-bold text-primary\">" . $this->title . "</h6></div>";
    }
...
#      $this->childstruct->parentForm = $form;
      $type = 'form';
      $cr = $analyze;
      Theme::JS('/libs/js/formity.add.js', 'formity.add');
...
#      $this->extra = 'autocomplete';
      ...
      Theme::JS('/libs/js/jquery-ui.min.js', 'jquery-ui');
      Theme::CSS('/libs/css/jquery-ui.min.css');

    } elseif($this->type == 'panel') {
      Theme::JS('/libs/js/formity.panel.js', 'formity.panel');

    } elseif($this->type == 'tree') {
      Theme::JS('/libs/js/formity.tree.js', 'formity.tree');
      ...
      Theme::JS('/libs/js/jquery.tagsinput.js');
      Theme::CSS('/libs/css/jquery.tagsinput.css');
    }
    ...
    if(!empty($cr)) {
      if(is_numeric($cr)) {
        ...
        Theme::JS('/libs/js/elements.add.js', 'elements.add');
      }
      $pcr = explode(':', $cr);
...
###############################
          ...
          if(!empty($this->getChildren())) { foreach($this->getChildren() as $k => $f) {
            ...
            $h .= '</tr>';
            $h .= '</tbody>';
            #$h .= static::_renderFormity($f, $nivel + 1);
          } }
          ...
          $h .= '</table>';
          if($this->childrangchange) {
            $h .= '<script>requireJS(\'elements.add\', function() { ElementsAdd({ clone: "#clonar_' . $keid . $this->getNameRequest() . '", contain: "#contenedorMore_' . $keid . $this->getNameRequest() . '", plus: "#addMore_' . $keid . $this->getNameRequest() . '", limit: 100, join: ".joinMore_' . $this->getNameRequest() . '" }).init(); });</script>';
          }
