...
class Tablefy {
  ...
  public function renderOnlyTable($attr = null, $basicHTML = false) {
        ...
        foreach($data as $tr) {
          ...
          if($this->options !== false && !$basicHTML) {
            $rp .= "<td data-label=\"Opciones\">\n";
              $rp .= "<div class=\"opciones columns\" style=\"justify-content: flex-end;\">\n";
              foreach($tr['tb_options'] as $o) {
                $rp .= "<a class=\"button is-small\" " . (!empty($o['popy']) ? 'data-popy' : '') . " href=\"" . $o['href'] . "\" title=\"" . $o['name'] . "\">" . $o['rotulo'] . "</a>\n";
              }
              $rp .= "</div>\n";
            $rp .= "</td>\n";
          }
    ...
    if(!is_null($this->pagination) && !$basicHTML) {
      if($this->ajax) {
        ...
        $this->pagination->link = Route::link(null, DOMINIO_ACTUAL, SUBDOMINIO_ACTUAL, $this->pagination->vkey . '=$p1&' . $this->passVarAjax);
      }
  ...
  public function renderInPage($attr = null) {
    ...
    $rp .= "<div class=\"card-body\"><div class=\"table-responsive\">";
