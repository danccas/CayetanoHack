...
function interpretar_respuesta(box, pf_id, pregunta_id, text) {
  ...
  $.ajax({
    ...
    success: function(x) {
      $(box).closest('.row').find('.respuesta_seleccionada').removeClass('respuesta_seleccionada');
      if(x.bool) {
	$(box).closest('.row').find("[data-resp='positivo'] a").click();//addClass('respuesta_seleccionada');
      } else {
	$(box).closest('.row').find("[data-resp='negativo'] a").click();//addClass('respuesta_seleccionada');
      }
    },
  });
  }, 400);
